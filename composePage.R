OSSComposePage <- function(type="rds"){
    ...
    page <- append(page,"<meta charset=\"windows-1251\">")
    ...
    for(i in 1:dim(oss_frame)[1]){
        ...
        if(subpart!=0){
            ...
            page <- append(page,paste0("<div class=\"act\" id=\"",chapter,".",act,"\">","<b>цКЮБЮ ",chapter,".",act,". ",text,"</b></div><hr>",collapse=""))
        } else if(chapter!=0){
            page <- append(page,paste0("<h2 class=\"chapter\" id=\"",chapter,"\">","пНГДЁК ",chapter,". ",text,"</h2><br>",collapse=""))
        }
    }
    page <- append(page,"</body>")
    page <- append(page,"</html>")
